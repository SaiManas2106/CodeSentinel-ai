...
async def _clone_or_pull(repo_url: str, target_path: Path, github_token: str) -> Path:
    ...
    if proc.returncode != 0:
        raise RuntimeError(
            f"Failed to clone repository: {stderr.decode('utf-8', errors='ignore')}"
        )
    return target_path
...
async def _ingest_repository_task(
    repo_id: str, repo_path: str = ""
) -> dict[str, str | int | float]:
    ...
    try:
        async with AsyncSessionLocal() as db:
            ...
            await redis_client.hset(
                f"ingestion:{repo_id}", mapping={"status": "cloning", "progress": "10"}
            )
            if repo_path:
                local_path = Path(repo_path)
            else:
                github_url = f"https://github.com/{repo.full_name}.git"
                local_path = await _clone_or_pull(
                    github_url,
                    tmp_dir / repo.full_name.replace("/", "_"),
                    repo.user.github_access_token if repo.user else "",
                )

            await redis_client.hset(
                f"ingestion:{repo_id}",
                mapping={"status": "embedding", "progress": "50"},
            )
            ...
            pipeline = RAGPipeline(
                redis_client=Redis.from_url(settings.redis.url), qdrant_client=qdrant
            )
        ...
        await redis_client.hset(
            f"ingestion:{repo_id}", mapping={"status": "failed", "error": str(exc)}
        )
        raise
    finally:
