...
class ReviewOrchestrator:
    ...
    async def _invoke_json(self, prompt: str, node_name: str) -> dict[str, Any]:
        ...
        for attempt in range(1, 4):
            try:
                ...
                logger.warning(
                    "agent.node.retry", node=node_name, attempt=attempt, error=str(exc)
                )
                if attempt == 3:
                    raise
    ...
    async def retrieve_context_node(self, state: ReviewState) -> ReviewState:
        ...
        elapsed_ms = round((time.perf_counter() - start) * 1000, 2)
        logger.info(
            "orchestrator.retrieve_context", chunks=len(chunks), elapsed_ms=elapsed_ms
        )
